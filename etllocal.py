...
songs_path          = config.get("LOCAL","songs_data") 
logs_path           = config.get("LOCAL","logs_data")
...
def main():
    ...
    songs_list = get_files(songs_path)
    songs_df = spark.read.schema(songs_sch).json(songs_list)
    
    logs_sch = get_logs_staging_schema()
    logs_list = get_files(logs_path)
    logs_df = spark.read.schema(logs_sch).json(logs_list)
